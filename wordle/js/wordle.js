document.addEventListener("DOMContentLoaded", () => {
  let letters = parseInt(new URLSearchParams(window.location.search).get("letters"));
  ...
  function restoreLocalStorage() {
    for (let index = 0; index < guesses; index++) {
      let storedEl = getLocalStorageElement("wordle_guess" + (index + 1))
      if (storedEl != null) {
        if (storedEl.length !== letters) {
          resetLocalStorage("wordle");
          return;
        }
    ...
    for (let index = 0; index < guesses * letters; index++) {
      ...
      let storedEl = getLocalStorageElement("wordle_tile" + (index + 1))
      if (storedEl != null) {
    ...
    for (let i = 0; i < keys.length; i++) {
      let storedKey = getLocalStorageElement("wordle_key_" + keys[i].textContent);
      if (storedKey != null) {
    ...
    word = getLocalStorageElement("wordle_word");

    let result = getLocalStorageElement("wordle_result");
    if (result === "win") {
      win();
    } else if (result === "loss") {
      loss();
  ...
  function win() {
    setLocalStorageElement("wordle_result", "win");
  ...
  function loss() {
    setLocalStorageElement("wordle_result", "loss");
  ...
  function getNewWord() {
    get('http://ec2-54-93-222-115.eu-central-1.compute.amazonaws.com:8080/wordle/get', {player_id: getLocalStorageElement("player_id"), word_length: letters})
      ...
      .then(response => {
        word = response.data
        setLocalStorageElement("wordle_word", word);
      })
            ...
            .then(response => {
              ...
              currentWordArr.forEach((letter, index) => {
                setTimeout(() => {
                  ...
                  setLocalStorageElement("wordle_tile" + letterId, tileColor);
                  ...
                  if (!(key.style.backgroundColor === greenRGB || (key.style.backgroundColor === yellowRGB && tileColor !== green))) {
                    ...
                    setLocalStorageElement("wordle_key_" + letterEl.textContent, tileColor)
                  }
                }, interval * index);
              });
              ...
              setLocalStorageElement("wordle_guess" + guessedWordCount, currentWord);
  ...
  document.getElementById("ver2").addEventListener('click', () => {
    resetLocalStorage("wordle");
  })
  document.getElementById("ver3").addEventListener('click', () => {
    resetLocalStorage("wordle");
  })
});
